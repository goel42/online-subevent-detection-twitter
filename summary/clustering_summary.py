...
# def get_focus_set_unique(cluster, focus_num,docs_tfidf):
# 	dist = []
# 	dist_uniq = OrderedDict()
# 	pts = cluster.points_indices
# 	# print("len of docs")
# 	# print(len(docs_tfidf))
# 	# print("points: ")
...
for key, cluster in cluster_list.items():
	...
	for f_idx in f_indices:
		# change for nepal dataset so as to only take text
		summ.append(docs[f_idx])

import lexrankfinal as lx

max_summ_words =int(sys.argv[2])
summary = lx.gen_lexrank_summary(summ,max_summ_words,docs_clean,upto)
print(summary)
